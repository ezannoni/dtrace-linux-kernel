...
retry:
	if (getline(&i->line, &i->line_size, i->f) < 0) {
		if (ferror(i->f)) {
			fprintf(stderr, "Error reading from modules_thick "
				"file: %s\n", strerror(errno));
			exit(1);
		}
		rewind(i->f);
	...
	*module_name = strdup(last_slash);
	...
	if (composite) {
		...
		for (npaths = 0, one_object = object_name;
		     one_object != NULL;
		     npaths++,
			     one_object = strchr(one_object + 1, ' '));
	}

	module_paths = malloc((npaths + 1) * sizeof (char *));
	if (!module_paths) {
		fprintf(stderr, "module_thick_iter_next: out of memory on "
			"module %s\n", *module_name);
		exit(1);
	}
	...
	if (composite) {
		...
		while ((one_object = strsep(&object_name, " ")) != NULL) {
			if (i >= npaths) {
				fprintf(stderr,
					"module_thick_iter_next: "
					"num_objs overflow on module "
					"%s: this is a bug.\n",
					*module_name);
				exit(1);
			}
