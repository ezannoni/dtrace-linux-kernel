...
/*
 * Convert 'long unsigned int' to 'sizetype'.  Internal use within type_id() only.
 */
			      ...
			      int flags)
{
	 ...
	 * (Never do this for base types, which don't have one, or for
	 * enumeration types, which CTF cannot represent the type of and where
	 * GCC can sometimes randomly choose not to emit the type DIE.)
	 *
	...
	case DW_TAG_array_type: {
		...
		do {
			char *sub_id = type_id_internal(&dim_die, overrides, fun, data,
